...
/*
  ...
  Max External IRQ (in mcu/freescale_mpc5777c/ee_mpc5777c.h)
...
__INLINE__ void __ALWAYS_INLINE__
    EE_mpc5777c_set_external_priority(EE_UREG psr, EE_BIT prio);
__INLINE__ void __ALWAYS_INLINE__
    EE_mpc5777c_set_external_priority(EE_UREG psr, EE_BIT prio)
{
	(*(volatile EE_UINT8 *) (INTC_BASE+0x40U+(psr))) =
		(volatile EE_UINT8)(CORE_MASK_SELECTOR | (prio));
}
