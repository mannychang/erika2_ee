...
/* Handler Declaration */
void EE_spc574k_system_timer_handler(void);

#ifdef EE_ISR_DYNAMIC_TABLE
void EE_spc574k_system_timer_handler(void)
#else
ISR2(EE_spc574k_system_timer_handler)
#endif
{
    /* Reset initial value */
    STM_CNT = 0U;
...
void EE_initialize_system_timer(void)
{
    ...
    EE_e200z7_register_ISR(16 + ISR_NUMBER,
		EE_spc574k_system_timer_handler, 1);
#endif /* EE_ISR_DYNAMIC_TABLE */

#if (EE_SYSTEM_TIMER_DEVICE == EE_SPC574K_STM)
    /* OSTICKDURATION is defined as nanoseconds macro */
    STM_CMP0 = ((EE_CPU_CLOCK + (OSTICKDURATION / 2000U)) /
